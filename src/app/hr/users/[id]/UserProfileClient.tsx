...
export default function UserProfileClient({ userId, profile, sheets: initialSheets }: { userId: string; profile: any; sheets: any[] }) {
  ...
  const handleValidate = async (sheetId: string) => {
    ...
    if (d.error) toast.error(d.error); else toast.success("Validada!");
  };
      ...
      <div className="card">
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-6">
          ...
          <div className="flex gap-8 text-right shrink-0">
            <div>
              <span className="block text-3xl font-bold font-mono text-brand-dark tracking-tight">{fM(totalMins)}</span>
              <span className="text-xs text-brand-muted tracking-wide uppercase">Horas totais</span>
            </div>
            <div>
              <span className="block text-3xl font-bold font-mono text-brand-dark tracking-tight">{sheets.length}</span>
              <span className="text-xs text-brand-muted tracking-wide uppercase">Folhas</span>
            </div>
            <div>
              <span className="block text-3xl font-bold font-mono text-brand-dark tracking-tight">{latestSheet ? format(new Date(latestSheet.week_start+"T00:00:00"), "dd/MM", {locale:pt}) : "—"}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="card !p-4 min-h-[200px]">
          {!selectedWeek ? (
            ...
            <div className="text-center py-16"><p className="text-brand-muted text-sm mb-2">Nenhuma folha esta semana</p><p className="text-xs text-brand-soft">{format(selectedWeek,"dd/MM",{locale:pt})} – {format(addDays(selectedWeek,6),"dd/MM/yyyy",{locale:pt})}</p></div>
          ) : (
          ...
          <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-lg mx-4 space-y-4 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2 mb-2">
                {workerProjects.map((p:any)=>(
                  <span key={p.id} className="text-xs bg-brand-gold/10 text-brand-dark font-medium px-2.5 py-1 rounded-full flex items-center gap-1">{p.name}</span>
                ))}
              </div>
              <select onChange={e => { if(e.target.value) { setWorkerProjects(prev => { if(prev.find(p=>p.id===e.target.value)) return prev; const p = allProjects.find(x=>x.id===e.target.value); return p?[...prev,p]:prev; }); e.target.value=""; } }} defaultValue="" className="input-field text-sm">
