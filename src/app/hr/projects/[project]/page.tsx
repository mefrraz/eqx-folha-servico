...
export default async function ProjectDetail({ params }: { params: { project: string } }) {
  ...
  const { data: sheets } = await supabase.from("work_sheets").select("*, work_entries(*), worker:profiles!work_sheets_worker_id_fkey(full_name)").eq("project_id", pid).order("week_start",{ascending:false}).limit(200);
  ...
  // Unique workers
  const workers = Array.from(new Map(safe.map(s => [s.worker_id, s.worker?.full_name])).entries()).map(([id, name]) => ({id, name}));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-brand-soft mt-1">
            {project.client?.name && <Link href={`/hr/clients/${project.client_id}`} className="hover:text-brand-gold transition-colors">{project.client.name}</Link>}
            {project.location && <span className="text-brand-muted"> · {project.location}</span>}
          </p>
          <p className="text-sm text-brand-soft mt-0.5">{wS.size} trabalhador{wS.size!==1?"es":""} · {safe.length} folha{safe.length!==1?"s":""} · {wC} semana{wC!==1?"s":""}</p>
        </div>
      ...
      {workers.length > 0 && (
        <div className="card">
          <h4 className="text-xs font-semibold text-brand-soft tracking-wide uppercase mb-3">Trabalhadores</h4>
          <div className="flex flex-wrap gap-2">
            {workers.map(w => (
              <Link key={w.id} href={`/hr/users/${w.id}`} className="text-sm bg-brand-gold/10 text-brand-dark font-medium px-3 py-1.5 rounded-full hover:bg-brand-gold/20 transition-colors">
                {w.name}
              </Link>
            ))}
          </div>
