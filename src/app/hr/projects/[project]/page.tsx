...
export default async function ProjectDetail({ params }: { params: { project: string } }) {
  ...
  return (<div className="space-y-6"><Link href="/hr/projects" className="text-xs text-brand-muted hover:text-brand-dark transition-colors">← Obras</Link><ProjectDetailClient project={project} sheets={sheets||[]} assignedCount={assignedCount} clients={clients||[]} /></div>);
}
