                  ...
                  <div key={s.id} className="border border-brand-light/30 rounded-xl">
                    ...
                    {isOpen && (
                      <div className="px-3 pb-3">
                        ...
                        <tbody>{["monday","tuesday","wednesday","thursday","friday","saturday"].flatMap(day=>{const dayEntries=(s.work_entries||[]).filter((x:any)=>x.day===day);if(dayEntries.length===0)return[];return dayEntries.map((e:any)=>{let dm=0;if(e.start_time&&e.end_time){const[a,b]=e.start_time.split(":").map(Number);const[c,d]=e.end_time.split(":").map(Number);dm=c*60+d-(a*60+b);}return(<tr key={`${day}-${e.shift}`} className={`border-b border-brand-light/20 ${e.shift==="afternoon"?"bg-blue-50/30":""}`}><td className="py-1 font-medium">{DL[day]}{" "}<span className={`text-[9px] ${e.shift==="afternoon"?"text-blue-600":"text-amber-600"}`}>{e.shift==="afternoon"?"T":"M"}</span></td><td className="py-1 text-brand-soft">{e.work_description||"—"}</td><td className="py-1 text-brand-muted">{WT[e.work_type]||"—"}</td><td className="py-1 text-right font-mono">{dm>0?formatMinutes(dm):"—"}</td></tr>);});})}</tbody></table>
