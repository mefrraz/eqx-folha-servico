...
export default function SignupPage() {
  const [fullName,setFullName]=useState("");const [email,setEmail]=useState("");const [password,setPassword]=useState("");const [inviteCode,setInviteCode]=useState("");const [loading,setLoading]=useState(false);
  ...
  const handle=async(e:React.FormEvent)=>{e.preventDefault();setLoading(true);
    ...
    if(!checkData.valid){toast.error("Código de convite inválido.");setLoading(false);return;}
  ...
  return(<div className="flex min-h-screen items-center justify-center bg-page px-4">
    <div className="w-full max-w-sm space-y-8">
      <div className="text-center"><img src="/eqx-logo.svg" alt="EQX" className="h-10 mx-auto mb-4"/><h2 className="text-lg font-semibold text-brand-dark">Criar conta</h2><p className="text-sm text-brand-soft mt-1">Registo de trabalhador EQX</p></div>
      <form onSubmit={handle} className="card space-y-4">
        ...
        <div><label htmlFor="inviteCode" className="label-field">Código de convite</label><input id="inviteCode" type="text" required value={inviteCode} onChange={e=>setInviteCode(e.target.value)} className="input-field" placeholder="Código fornecido pela EQX"/></div>
