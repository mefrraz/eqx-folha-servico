...
export default async function WorkerDashboard() {
  ...
  return(<div className="space-y-6">
    ...
    <div><h3 className="text-sm font-semibold text-brand-soft tracking-wide uppercase mb-3">Folhas anteriores</h3>{!sheets||sheets.length===0?<div className="card text-center py-10 text-brand-muted text-sm">Nenhuma folha.</div>:<div className="grid gap-2">{sheets.map(s=>{const m=calcMinutes(s.work_entries||[]);return(<Link key={s.id} href={`/worker/sheet/${s.id}`} className="card !p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 hover:border-brand-gold/40 transition-all"><div><p className="font-medium text-brand-dark text-sm">Semana {format(new Date(s.week_start+"T00:00:00"),"dd/MM",{locale:pt})}–{format(new Date(s.week_end+"T00:00:00"),"dd/MM/yy",{locale:pt})}</p><p className="text-xs text-brand-soft">{s.client||"—"} · {s.work_number||"—"}</p></div><div className="flex items-center gap-3"><span className="font-mono text-sm text-brand-dark">{formatMinutes(m)}</span><span className={s.status==="draft"?"badge-draft":s.status==="submitted"?"badge-submitted":"badge-reviewed"}>{STATUS_LABELS[s.status]||s.status}</span></div></Link>);})}</div>}</div>
  </div>);
}
